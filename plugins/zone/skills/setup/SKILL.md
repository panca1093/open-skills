---
name: setup
description: One-time configuration for the Zone plugin — prompts for Notion DB/page IDs and the local MyBook wiki path, writes them to ~/.claude/plugins/data/zone/config.json. Triggers when the user asks to "configure zone", "set up zone", "zone setup", or runs /zone:setup. Safe to re-run to update settings.
argument-hint: "[--show] [--reset]"
allowed-tools: [Read, Write, Bash, AskUserQuestion]
---

# /zone:setup — Plugin Configuration

Zone reads its configuration from `~/.claude/plugins/data/zone/config.json`. This skill creates or updates that file interactively. Run once after installing the plugin; re-run any time to change settings.
...
```bash
CONFIG_DIR="$HOME/.claude/plugins/data/zone"
...
if [ -f "$CONFIG_PATH" ]; then
  ...
  echo "No config yet. Run /zone:setup to create one."
fi
```
...
```
Zone config written to ~/.claude/plugins/data/zone/config.json

Notion sync: <enabled if either DB ID is set; "disabled — run /zone:setup to add IDs" otherwise>
Wiki path:   <wiki_path>

Run `/zone TICKET-XXX` (Jira) or `/zone` (Scratch) to start.
```
